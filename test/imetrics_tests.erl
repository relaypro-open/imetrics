...
%% add tests
add_test_() ->
    {setup, fun start/0, fun stop/1, fun add_test/1}.
...
add_test(_Fixture) ->
    ...
    [
        ...
        ?_assertEqual(
            {error, {function_clause, check_inputs}}, imetrics:add({a, b, c, d, e, f, g, h, i})
        ),
        ?_assertEqual({error, {badmatch, check_inputs}}, imetrics:add(bad, map))
    ].

gauge_test_() ->
    {setup, fun start/0, fun stop/1, fun gauge_test/1}.
...
multigauge_test(_) ->
    ...
    [
        ?_assertEqual(
            [{<<"$dim">>, <<"dim">>}, {<<"test">>, 3}],
            proplists:get_value(<<"multigauge_test">>, Gauges)
        )
    ].
...
dimension_test(_) ->
    ...
    [
        ?_assertEqual(
            [
                {<<"dimension_test1">>, [{<<"$dim">>, <<"dim1">>}]},
                {<<"dimension_test2">>, [{<<"$dim">>, <<"dim2">>}, {<<"key2">>, 1}]}
            ],
            imetrics:get_counters()
        )
    ].

%% badarg tests
badarg_test_() ->
    {setup, fun() -> ok end, fun(_) -> ok end, fun badarg_test/1}.
...
%% get tests
get_test_() ->
    {setup, fun start_get/0, fun stop/1, fun get_test/1}.
...
%% http tests
http_test_() ->
    {setup, fun start_http/0, fun stop/1, fun http_test/1}.
...
%% tick/tock tests
ticktock_test_() ->
    {setup, fun start_ticktock/0, fun stop/1, fun ticktock_test/1}.
...
ticktock_s_test_() ->
    {setup, fun start_ticktock/0, fun stop/1, fun ticktock_s_test/1}.
